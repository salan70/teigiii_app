...
1. **既存の Ds コンポーネント / variant を検索する。**
   `mobile_app/lib/core/design_system/component/` を Glob で一覧し、Grep で用途に近い型・名前付きコンストラクタを探す。
   既存 variant で足りるなら新規実装しない。
2. **Widgetbook use case と仕様を確認する。**
   `just mobile-widgetbook` でカタログを起動し、候補コンポーネントの light / dark・状態・長文ケースを見る。
...
ローカル限定（CI では実行しない。実行できない環境ではその旨を報告する）:
