...
- `POST /v1/words` は結果種別 `registrationResult` を返す。新規作成は `created`、既存行だが登録前は閲覧者にとって公開経路に出ていなかったものは `promoted`、登録前から公開経路に出ていたものは `alreadyPublic`。判定は明示登録を行う前の公開状態で決め、ミュートを含む閲覧者基準とする
- `GET /v1/words/lookup` は登録前の既存語チェック専用で、正規化後の `(表記, よみ)` 完全一致かつ閲覧者にとって公開されている言葉だけを返す。非公開の言葉は存在を秘匿して `null` を返す。よみの文字種は検証しない
