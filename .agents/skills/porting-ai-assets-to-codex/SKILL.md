...
- Claude hooks は Codex へ直接移植しない。
- 承認 UI や権限昇格は Codex の実行環境に従う。

## 検証

```bash
find .agents AGENTS.md -maxdepth 4 -type l -ls
rg -n "ask_user_input|Task tool|Claude Code|\\.claude/hooks|\\.agents/skills.*symlink|正本は `.claude`" \
  .agents AGENTS.md
```
